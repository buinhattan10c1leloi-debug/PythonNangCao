...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    # PHÂN QUYỀN
    role: so.Mapped[str] = so.mapped_column(sa.String(20), default='patient')
    ...
    # QUAN HỆ CHO BỆNH VIỆN
    appointments_as_patient: so.WriteOnlyMapped['Appointment'] = so.relationship(
        foreign_keys='Appointment.patient_id', back_populates='patient')
    appointments_as_doctor: so.WriteOnlyMapped['Appointment'] = so.relationship(
...
class Appointment(db.Model):
    ...
    timestamp: so.Mapped[datetime] = so.mapped_column(
        index=True, default=lambda: datetime.now(timezone.utc))
    appointment_date: so.Mapped[date] = so.mapped_column(index=True)
    appointment_time: so.Mapped[dt_time]
    notes: so.Mapped[Optional[str]] = so.mapped_column(sa.Text)
    status: so.Mapped[str] = so.mapped_column(sa.String(20), default='Pending') 
    
    patient_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(User.id), index=True)
    doctor_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(User.id), index=True)

    patient: so.Mapped[User] = so.relationship(
        foreign_keys=[patient_id], back_populates='appointments_as_patient')
    doctor: so.Mapped[User] = so.relationship(
        foreign_keys=[doctor_id], back_populates='appointments_as_doctor')
    
    medical_record: so.Mapped[Optional['MedicalRecord']] = so.relationship(
        back_populates='appointment', uselist=False)


class MedicalRecord(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    diagnosis: so.Mapped[str] = so.mapped_column(sa.Text)
    prescription: so.Mapped[str] = so.mapped_column(sa.Text)
    doctor_advice: so.Mapped[Optional[str]] = so.mapped_column(sa.Text)
    timestamp: so.Mapped[datetime] = so.mapped_column(
        default=lambda: datetime.now(timezone.utc))
